    ...
    <div className='min-h-screen bg-white border-r'>
...

      {
        dtoken && <ul className='text-[#515151] mt-5'>
          ...
          <NavLink className={({isActive})=>`flex items-center gap-3 py-3.5 px-3 md:px-9 md:min-w-72 cursor-pointer ${isActive?'bg-[#f2f3ff] border-r-4 border-[#5f6fff]':''}`} to={'/doctors-appointments'}>
            <img src={assets.appointment_icon} alt='image'/>
            <p className='hidden md:block'>Appointments</p>
          </NavLink>
          <NavLink className={({isActive})=>`flex items-center gap-3 py-3.5 px-3 md:px-9 md:min-w-72 cursor-pointer ${isActive?'bg-[#f2f3ff] border-r-4 border-[#5f6fff]':''}`} to={'/doctors-profile'}>
            <img src={assets.people_icon} alt='image'/>
